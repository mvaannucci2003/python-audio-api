...
Output format: CSV with columns: tag, domain, source, environment, scenario
...
6.	Generate exactly {SCENARIOS_PER_TAG_PER_DOMAIN} scenarios per tag per domain ({SCENARIOS_PER_TAG_PER_DOMAIN * 6} rows per tag).
7.	Scenario uniqueness is mandatory across the ENTIRE output. No two rows may have identical or near-identical scenario text, regardless of whether they share the same tag, domain, or any other column. Before generating any scenario, verify it has not already been used elsewhere in the output.
